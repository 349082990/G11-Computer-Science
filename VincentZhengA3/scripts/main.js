...
function addMonster(){
    ...
    if (PERMISSIONS[currentUser].canAddInfo){
        ...
        // Initalize an empty vaariable for existingMonsters
        let existingMonster;
        
        // Iterate through the monsters array 
        for (let i = 0; i < monsters.length; i++){
            // Find if any index in the monsters array matches the info in the newMonster object
            if (monsters[i].name.toLowerCase() === newMonster.name.toLowerCase() || monsters[i].health === newMonster.health && monsters[i].name.toLowerCase() === newMonster.name.toLowerCase() && monsters[i].type.toLowerCase() === newMonster.type.toLowerCase()){
                // Set the existingMonster array to the current monsters index and breka the code. If the loop continues without finding a match for anything, then it will be undefined
